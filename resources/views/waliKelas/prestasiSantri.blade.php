...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <x-alert />
        <div class="card mb-4">
            <h5 class="card-header">Tambah Prestasi</h5>
            <div class="card-body">
                <div>
                    <form enctype="multipart/form-data" action="{{ route('prestasi-simpan') }}" method="POST">
                        @csrf
                        <div class="mb-3">
                            <input type="hidden" name="data_raport_k13_id" value="{{ $data->id }}">
                            <label class="form-label" for="basic-default-fullname">Nama Prestasi</label>
        ...
        <div class="card">
            <h5 class="card-header ">Daftar Prestasi {{ $santri->namaLengkap }}</h5>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            $('#exampleModalCenter').on('show.bs.modal', function(event) {
                ...
                var modal = $(this)
                modal.find('.modal-text').text('Yakin ingin menghapus prestasi ' + nama + ' ?')
                document.getElementById('formHapus').action = '/data-raport/prestasi/delete/' + id;
            })
        });
    </script>
@endsection
