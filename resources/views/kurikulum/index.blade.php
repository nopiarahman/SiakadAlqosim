...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <x-alert />
        @role('Super-Admin')
            <a href="{{ url('/kurikulum/tambah') }}" type="button" class="btn btn-primary mb-3"> Tambah
                Kurikulum Baru
            </a>
        @endrole
        <!-- Basic Bootstrap Table -->
        <div class="card">
            <h5 class="card-header">Daftar Kurikulum</h5>
            <div class="text-nowrap table-responsive">
                <table class="table  m-3" id="table">
                    ...
                    <tbody class="table-border-bottom-0">
                        @foreach ($kurikulum as $i)
                            <tr>
                                <td>{{ $loop->iteration }}</td>
                                <td><i class="fab fa-angular fa-lg text-danger"></i>
                                    <strong>{{ ucfirst($i->nama) }}</strong>
                                </td>
                                <td>
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu">
                                            <a class="dropdown-item"
                                                href="{{ route('edit-kurikulum', ['id' => $i->id]) }}"><i
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLongTitle">Hapus Kurikulum </h5>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            $('#exampleModalCenter').on('show.bs.modal', function(event) {
                ...
                var modal = $(this)
                modal.find('.modal-text').text('Yakin ingin menghapus kurikulum ' + nama + ' ?')
                document.getElementById('formHapus').action = '/kurikulum/delete/' + id;
            })
        });
    </script>
@endsection
